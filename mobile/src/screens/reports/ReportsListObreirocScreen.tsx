...
import { deleteCell, getAllCellsForObreiro } from '../../services/cellService';
...
export const ReportsListObreirocScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCells = async () => {
      if (user?.id) {
        setLoading(true);
        try {
          const data = await getAllCellsForObreiro(user?.id);
