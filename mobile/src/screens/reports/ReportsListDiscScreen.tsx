...
import { format, parseISO, startOfMonth, endOfMonth, isSameMonth, isFuture, isBefore } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { Ionicons } from '@expo/vector-icons';

import AuthContext from '../../context/UserContext';
import theme from '../../styles/theme';
import { deleteCell, getAllCellsForDisc } from '../../services/cellService';
...
export const ReportsListDiscScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCells = async () => {
      if (user?.id) {
        setLoading(true);
        try {
          const data = await getAllCellsForDisc(user?.id);
          console.log("Dados recebidos da API:", data);  // Adicione este log para inspecionar a resposta
  ...
  useEffect(() => {
    ...
    const currentIndex = months.findIndex((month) => isSameMonth(month, selectedMonth));
    const offset = currentIndex >= 0 ? currentIndex * 108 : 0;
    scrollViewRef.current?.scrollTo({ x: offset, animated: true });
  }, [selectedMonth]);
